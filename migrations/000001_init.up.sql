-- Migration 000001: initial schema, chuyển thể từ
-- docs/appendices/vm-factory-schema-v1.0.sql (blueprint v1.1).
--
-- Đã bỏ CREATE TABLE schema_migrations khỏi bản blueprint gốc: golang-migrate
-- (công cụ migration đã chọn ở tài liệu 11 mục 2.7) tự quản lý một bảng
-- schema_migrations riêng (version BIGINT, dirty BOOLEAN) để track version đã
-- apply — giữ cả hai sẽ đụng tên bảng. Không có quyết định kiến trúc nào bị
-- mất: mục đích audit "ai/khi nào áp migration" vẫn có qua audit_events +
-- lịch sử git của thư mục migrations/ này.
